 ...
 */
public class MUFSchedulingPlan extends SchedulingPlan {
  public static final String WORKFLOW_SCHEDULER_XML = 
                             "workflow-scheduler.xml";
  public static final String FACTOR_PROPERTY_NAME = 
                      "mapred.workflow.urgencyFactor";
  ...
  public MUFSchedulingPlan() {
    this.wConf = new Configuration(false);
    this.wConf.addResource(WORKFLOW_SCHEDULER_XML);
    // don't understand why hadoop-1.2.1 removes the getDouble
    // method from Configuration.java. anyway, let me stick to it
    this.uFactor = wConf.getFloat(FACTOR_PROPERTY_NAME, 1.0F);
  }
  ...
  public boolean generatePlan(int maxSlots, 
    ...
    try {
      ...
      if (slotNum > 0 && null != schedEvents) {
        int size = schedEvents.size();
        double step = (this.uFactor - 1) / size;
        double factor = 1;
        System.out.println("^^^^^^^ uFactor is : " + this.uFactor + 
                           ", step is : " + step);
        for (SchedulingEvent schedEvent : schedEvents) {
          schedEvent.schedRequirement = 
            (long)(schedEvent.schedRequirement * factor);
          factor = factor + step;
          System.out.print(schedEvent.schedRequirement + ", ");
        }
        System.out.println();
        return true;
      }
