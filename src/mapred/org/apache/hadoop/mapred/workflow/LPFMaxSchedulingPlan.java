 ...
 */
public class LPFMaxSchedulingPlan extends SchedulingPlan {

  private ArrayList<SchedulingEvent> schedEvents;
  private int slotNum;

  public LPFMaxSchedulingPlan() {
  }
  ...
  public boolean generatePlan(int maxSlots, 
    ...
    try {
      ...
      while (queue.size() > 0) {
        ...
        //calculcate its weighted length
        double maxLen = 0;
        if (deps.contains(jobName)) {
          for (String dep : deps.get(jobName)) {
            WJobConf depConf = wjobs.get(dep);
            if (maxLen < depConf.getPriority()) {
              maxLen = depConf.getPriority();
            }
          }
        }
        double selfLen = 
          jobConf.getMapNum() * jobConf.getMapEstTime() +
          jobConf.getRedNum() * jobConf.getRedEstTime() + maxLen;
      ...
      ArrayList<SchedulingEvent> curSched = 
        WorkflowUtil.checkFeasibility(wfConf, deps, preCounts, maxSlots);
      if (null == curSched) {
