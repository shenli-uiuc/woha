 ...
 */
class FairWorkflowScheduler extends TaskScheduler {
  ...
  protected FairWorkflowListener fairWorkflowListener;
  protected EagerTaskInitializationListener eagerTaskInitializationListener;
  private float padFraction;
  
  public FairWorkflowScheduler() {
    this.fairWorkflowListener = new FairWorkflowListener();
  }
  
  @Override
  public synchronized void start() throws IOException {
    super.start();
    taskTrackerManager.addJobInProgressListener(fairWorkflowListener);
    taskTrackerManager.addWorkflowInProgressListener(fairWorkflowListener);
  ...
  public synchronized void terminate() throws IOException {
    if (fairWorkflowListener != null) {
      taskTrackerManager.removeJobInProgressListener(
          fairWorkflowListener);
  ...
  public synchronized List<Task> assignTasks(TaskTracker taskTracker)
    ...
    for (int i=0; i < availableMapSlots; ++i) {
      ...
      state = NOT_ASSIGNED;
      queue = fairWorkflowListener.getMapQueue();
      synchronized (queue) {
          ...
          if (obj instanceof JobInProgress) {
            ...
            for (String activeJobName : wfStatus.getActiveJobs()) {
              ...
              if (NOT_ASSIGNED != state) {
                LOG.info("Shen Li mbf log: submitter "
                         + activeJobName + ", " +
                         assignedTasks.get(
            ...
            if (NOT_ASSIGNED == state) {
              ...
              for (String name : submittedJobs) {
                ...
                if (NOT_ASSIGNED != state) {
                  ...
                  WJobConf conf = 
                    wip.getConf().getWJobConfs().get(name);
                  wfStatus.addSchedWork(
                      schedTaskNum * conf.getMapEstTime());
                  break;
                }
              }
            }
/*
    ...
    for (int i = 0; i < availableReduceSlots; ++i) {
      state = NOT_ASSIGNED;
      queue = fairWorkflowListener.getRedQueue();
      synchronized (queue) {
        for (Object obj : queue) {
          if (obj instanceof JobInProgress) {
            ...
            for (String name : submittedJobs) {
              ...
              if (ASSIGNED == state) {
                ...
                WJobConf conf =
                  wip.getConf().getWJobConfs().get(name);
                wfStatus.addSchedWork(
                    schedTaskNum * conf.getRedEstTime());
                break;
              }
            }
          }
