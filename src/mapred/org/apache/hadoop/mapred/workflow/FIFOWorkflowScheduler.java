 ...
 * by default).
 */
class FIFOWorkflowScheduler extends TaskScheduler {
          ...
          if (obj instanceof JobInProgress) {
            ...
            for (String activeJobName : wfStatus.getActiveJobs()) {
              ...
              if (NOT_ASSIGNED != state) {
                LOG.info("Shen Li fifo log: submitter "
                         + activeJobName + ", " +
                         assignedTasks.get(
            ...
            if (NOT_ASSIGNED == state) {
              ...
              for (String name : submittedJobs) {
                ...
                if (NOT_ASSIGNED != state) {
                  ...
                  LOG.info("Shen Li fifo log: map " + name + ", " +
                           assignedTasks.get(
                             assignedTasks.size() - 1).getTaskID());
                  break;
                }
        ...
        for (Object obj : queue) {
          if (obj instanceof JobInProgress) {
            ...
            for (String name : submittedJobs) {
              ...
              if (ASSIGNED == state) {
                LOG.info("Shen Li fifo log: reduce " + name + ", " +
                         assignedTasks.get(
                           assignedTasks.size() - 1).getTaskID());
                break;
              }
