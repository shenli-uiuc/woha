...
public class JobInProgress {
  ...
  /**
   * Shen Li: default_conf is this.conf of JobTracker. use jobInfo
   * to get the real JobConf
   */
  ...
  throws IOException, InterruptedException {
    try {
      ...
      // Shen Li: here the JobConf contains WorkflowID information
      ...
      // Shen Li: add workflowID to status
      this.status.setWorkflowID(conf.getWorkflowID());
  ...
  /**
   * Shen Li: get job name, used to map job to wjobs (workflowid + jobName)
   */
  ...
  /**
   * Shen Li: EDF scheduler need job conf to get its
   * deadline
   */
  ...
  /**
   * Shen Li: 
   * Get a specific map task split with the given inner id. This is only
   * used for the submitter jobs.
   *
   * WorkflowScheduler call this to assign a task.
   */
  public synchronized Task obtainMapTaskByID(
                              TaskTrackerStatus tts,
                              int innerID) 
      throws IOException {
    if (!tasksInited) {
      LOG.info("Shen Li: Cannot create task split for " 
               + profile.getJobID());
      try { throw new IOException("state = " + status.getRunState()); }
      catch (IOException ioe) {ioe.printStackTrace();}
      return null;
    ...
    if (0 == numMapTasks) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("Shen Li: No maps to schedule for " 
                  + profile.getJobID());
      }
      return null;
    }

    if (innerID >= numMapTasks) {
      LOG.info("innerID " + innerID + " exceeds the limit " + numMapTasks);
      return null;
    }

    // TODO: it does not matter for now to call this method the second time
    // if the WorkflowScheduler does not receive the wjob after the first
    // call + some delay, as failMap(...) simply calls failedMaps.add(tip)
    // to add it to the set.
    
    Task result = maps[innerID].getTaskToRun(tts.getTrackerName());
    if (result != null) {
      scheduleMap(maps[innerID]);
      addRunningTaskToTIP(maps[innerID], result.getTaskID(), tts, true);
    }
    return result;
  }
